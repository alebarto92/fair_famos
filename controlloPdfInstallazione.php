...
$query="SELECT *,DATE_FORMAT(v.data_fine,'%d/%m/%Y') as data_intervento_formatted,DATE_FORMAT(v.data_inizio,'%d/%m/%Y') as data_inizio_visita_formatted,s.email as email_sede, 
s.persona_di_riferimento as persona_di_riferimento_sede,s.telefono as telefono_sede ,c.Mail as email_cliente, 
c.Referenti as persona_di_riferimento_cliente,s.CAP as capsede FROM $tbattivita v JOIN $tbutenti u ON u.id_user=v.id_dipendente JOIN $tbsedi s ON v.id_sede=s.id JOIN $tbclienti c ON c.id=s.id_cliente WHERE v.id=?";
...
if ($VISITA['nr_certificato']==0) {
    ...
    $query="UPDATE $tbattivita SET nr_certificato=? WHERE id_attivita=?";
    $stmt=$dbh->prepare($query);
...
$nomecliente=$VISITA['Cliente']." - SEDE: ".$VISITA['sede'];
...
join pcs_sedi_clienti on pcs_sedi_clienti.id=pcs_attivita_clean.id_sede 
left join pcs_aree on pcs_aree.id_sede=pcs_sedi_clienti.id 
left join pcs_postazioni on pcs_postazioni.id_area=pcs_aree.id
LEFT join pcs_tipo_postazione on pcs_tipo_postazione.id_tipo_postazione=pcs_postazioni.Tipo 
LEFT join pcs_modello_postazione on pcs_modello_postazione.id_modello_postazione=pcs_postazioni.Modello 
LEFT JOIN pcs_prodotto_postazione on pcs_prodotto_postazione.id_prodotto_postazione=pcs_postazioni.Prodotto 
left join pcs_tipi_servizio ON pcs_tipi_servizio.id=pcs_aree.Servizio 
...
WHERE pcs_attivita_clean.id=? order by pcs_aree.id,pcs_aree.Servizio,CAST(nome as SIGNED INTEGER),nome";
$stmt=$dbh->prepare($query);
$stmt->execute(array($codicevisita));
$SERVIZI=array();
