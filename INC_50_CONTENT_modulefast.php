    ...
    <div id="ModalDetails" class="modal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="blue bigger">COPIA AREE</h4>
                </div>
                ...
                <table id="dynamic-table" class="table table-striped table-bordered bootstrap-datatable datatable responsive">
                    ...
                    <tr>
                        ...
                        <?php if ($modulo['nome_modulo']=='Polizze') { ?>
                            <th><?php echo _("View");?>_<?php echo _("Actions");?>_<?php echo _("Allegati");?></th>
                        <?php } else { ?>
...
//additional functions for data table
        ...
        $(document).on("click",".aprimodal-dettagli",function(){
            ...
            $('#modal-body-ModalDetails').load('ajax_getmodulocopiaaree.php?'+ $.param({
        ...
        $( document ).on( "click", ".rinnovapolizze-button", function() {
            var idpolizza=$(this).attr("idpolizza");
            var params={};
            params.idpolizza=idpolizza;

            bootbox.confirm("<?php echo _('La polizza attuale verrà chiusa e verrà generata una nuova polizza con la data di decorrenza uguale alla data di scadenza della polizza attuale. Tutti i dati della attuale polizza saranno ricopiati sulla nuova polizza, ma poi dovrai salvarla e generare i pagamenti manualmente. Vuoi procedere?');?>", function(result) {
                if (result) {
                    $.ajax({
                        dataType: "json",
                        type: 'POST',
                        url: "ajax_rinnovapolizza.php",
                        data: jQuery.param(params) ,
                        success: function (data) {
                            console.log(data);
                            if (data.result==true) {
        ...
        }); // fine $( document ).on( "click", ".rinnovapolizze-button", function() {
        ...
        $(".replicatrattamento").click(function(){
            var idele=$(this).attr("idmodalele");
            var idmod=$(this).attr("idmodalmod");
